...
namespace leaves { namespace math 
{
	...
	>
	void evaluate(vector_expression<E1> const& e1, vector_expression<E2> const& e2)
	{
		...
		typedef vector_detail::evaluate_vector_with_vector <
			function_type, left_expression_type, right_expression_type > evaluation_type;

		evaluation_type::apply(get_expression(e1), get_expression(e2));
	}
	...
	>
	auto evaluate(vector_expression<E> const& e, T scalar)
		-> std::enable_if_t<is_scalar<T>::value>
	{
		...
		typedef vector_detail::evaluate_vector_with_scalar <
			function_type, value_type, scalar_type > evaluate_type;
